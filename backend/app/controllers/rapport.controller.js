const { Rapport} = require("../models/rapport.model.js");
...
exports.stats = function (req, res) {
    Rapport.find({idSoc: req.params.idSoc}).countDocuments().then(rapports => {
...
exports.findRapportByIdSoc = function(req , res) {
    ...
    Rapport.find({idSoc: req.params.idSoc}).populate({path: "marchandiseur",select:"name",model: "Marchandiseur"}).populate({path: "tache",select:"TitreT , marchandiseur",model: "Tache"})
     .then(rapports => {
        res.send(rapports);
    }).catch(err => {
        res.status(500).send({
...
exports.findOne = (req, res) => {
    Rapport.findById(req.params.rapportId).populate({path: "tache",select:"TitreT , marchandiseur",model: "Tache"}).populate({path: "marchandiseur",select:"name",model: "Marchandiseur"})
        .then(rapport => {
            if (!rapport) {
